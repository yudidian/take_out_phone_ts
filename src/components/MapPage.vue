<template>
	<div class="map-content">
		<Search class="map-search" shape="round" v-model="addressInfo" placeholder="请输入搜索关键词" />
		<div class="map">
			<el-amap ref="map" :center="center" :zoom="zoom" @init="init" />
		</div>
		<div class="address-list">
			<Cell style="width: 100%" v-for="item in addressList" :key="item.id" :title="item.address" @click="changAddress(item)" />
		</div>
		<MyLoading :show="show" />
	</div>
</template>
