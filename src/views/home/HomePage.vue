<template>
	<header>
		<Swipe class="my-swipe" :autoplay="3000" indicator-color="white">
			<SwipeItem v-for="item in swiperList" :key="item.id">
				<img :src="IMG_URL + item.imageUrl" alt="" />
			</SwipeItem>
		</Swipe>
		<div class="main">
			<div class="go-mine">
				<img id="show-home-header" src="../../assets/image/user.png" @click="isShowHeader" alt="kola" />
			</div>
					...
					<div class="title-name">
						...
						<ul class="description">
							<li class="description-item" @click="getDistance">
								<Icon name="location-o" v-if="distance === 0"> 点击获取距离 </Icon>
	...
	<main :class="showHeader ? 'top-40-main' : 'top-0-main'">
		<div class="type">
			<ul class="type-list" ref="listScroll">
				<li
		...
		<div class="dish">
			<ConfigProvider :theme-vars="themeVars">
				...
				>
					...
					<template #num>
						<Button type="primary" size="mini" round v-if="item.flavors" data-flag="no"> 选择规格 </Button>
						<Button icon="plus" type="primary" size="mini" data-flag="no" round v-else />
					</template>
				</Card>
			</ConfigProvider>
		</div>
	</main>
	<TasteSelection v-if="showDialog" title="选择规格" @hide="showDialog = $event" :dish="dish" :dish-type="dishType.toString()" />
</template>
...
const dishList = ref<Array<DishList>>([]);
...
const { getUserPosition, computedLine, distance, center, getAddress, formattedAddress } = useMap();
onMounted(() => {
	sendCategory();
	getSwiperList();
});
...
// 跳转详情页
const toDetails = (item: any, e: any) => {
...
// 点击菜单列表处理
const changeCategory = (item: any, index: any) => {
...
// 根据菜单id 获取对应菜品信息
const getDishById = async (categoryId: string) => {
...
// 根据菜单id 获取对应套餐信息
const getSetMealById = async (categoryId: string) => {
