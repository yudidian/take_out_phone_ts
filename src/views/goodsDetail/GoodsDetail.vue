			...
			<div class="content">
				...
				<div class="price">
					<div class="price-left">
						<div class="left-info">
							<div class="sort">月销售量 {{ goodsInfo?.salesVolume }}</div>
							<div class="number">￥{{ (goodsInfo?.price / 100).toFixed(2) }}</div>
						</div>
					</div>
					<div class="price-right">
						<Button type="primary" size="mini" round v-if="goodsInfo?.flavors" @click="chooseFlavors()"> 选择规格 </Button>
...
import { sendChangeFavorites, sendGetFavorites, sendGoodsInfo, sendGetDishDescription, sendSetmealInfo, sendAddCart } from "@/api/module/goods";
import { useCart } from "@/hooks/useCart";
import { DescriptionInfo, GoodsInfo } from "@/views/types/interface";
...
// 商品描述信息
const descriptionInfo = ref<DescriptionInfo>();
// 商品信息
...
// 购物车列表数据
onMounted(async () => {
	let getGoodsInfo: Promise<BaseResponse>;
	if (route.query.type === "1") {
		getGoodsInfo = sendGoodsInfo(route.params.id as string);
	} else {
		getGoodsInfo = sendSetmealInfo(route.params.id as string);
	}
	const res = await Promise.allSettled([
		getGoodsInfo,
		sendGetFavorites(route.params.id as string, route.query.type as string),
		getCartList(),
		sendGetDishDescription(route.params.id as string)
	]);
	setGoodsInfo(res);
});
// 处理商品信息
const setGoodsInfo = (res: any) => {
