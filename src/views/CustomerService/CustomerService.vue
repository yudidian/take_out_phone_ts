<template>
	<div class="chart-wrapper">
		...
		<div class="message-content" ref="messageContent">
			<Empty v-if="messageList.length === 0" :image="IMG_URL + 'empty.png'" description="暂无消息" />
			<div v-else class="message-item" v-for="item in messageList" :key="item.id">
				<div v-if="item.managerName !== undefined" class="left-review">
					<img :src="IMG_URL + item.managerAvatar" alt="客服头像" />
				...
				<div class="right-review" v-else>
					...
					<img :src="IMG_URL + item.userAvatar" alt="用户头像" />
				</div>
			</div>
		</div>
		<div class="bottom">
			<Field v-model="message" type="textarea" center clearable placeholder="请输入内容">
				<template #button>
...
<script name="CustomerService" setup lang="ts">
