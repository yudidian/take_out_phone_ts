		...
		<header>
			...
			<div class="user">
				<div class="img-wrapper">
					<img alt="用户头像" v-if="!userInfo.avatar" src="@/assets/image/headPage.png" />
					<img
						alt="用户头像"
						v-else
						:src="BASE_IMAGE_URL + userInfo.avatar"
						@click="ImagePreview([BASE_IMAGE_URL + userInfo.avatar])"
						crossOrigin="anonymous"
					/>
				</div>
				<span class="user-name">{{ userInfo.name }}</span>
				<img v-if="userInfo.sex === '1'" src="@/assets/image/woman.png" class="user-icon" alt="用户头像" />
				<img v-else src="@/assets/image/man.png" class="user-icon" alt="用户头像" />
			</div>
		</header>
		<article>
			<section>
				<CellGroup inset>
					<Cell is-link to="/pendingReceipt">
						<template #title>
							<Badge :content="orderSize" max="99">
								<Icon name="shop-collect-o" size="16" />
								<span class="custom-title">待收货</span>
							</Badge>
						</template>
					</Cell>
					<Cell icon="star-o" title="我的收藏" is-link to="/storeUp" />
					<Cell icon="records" title="历史订单" is-link to="/history/orders" />
				</CellGroup>
			</section>
			<section>
				<CellGroup inset>
					<Cell title="最新订单" style="height: 285px">
						<template #label>
			...
			<section>
				<Icon name="setting-o" class="btn-exit" @click="$router.push('/setting')" />
			</section>
		</article>
	</div>
</template>
...
import { OrderDetailList } from "@/views/types/interface";
const BASE_IMAGE_URL = import.meta.env.VITE_LOCAL_SERVE_IMGE_URL;
const userInfo = ref({});
const latestOrderList = ref<Array<OrderDetailList>>([]);
