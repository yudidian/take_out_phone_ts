<template>
	<section>
		<NavBar title="地址详情" left-text="返回" left-arrow @click-left="$router.back()" />
		<Empty
...
import { AddressListType } from "@/views/types/interface";
import { AddAddress } from "@/views/home/interface/type";
onMounted(() => {
	sendAddressList();
});
const router = useRouter();
const addressList = ref<Array<AddressListType>>([]);
const onEdit = (item: AddressListType) => {
	router.push({
		name: "AddressAdd",
		query: {
			id: item.id
