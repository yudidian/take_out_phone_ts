<template>
	...
	<div class="content" v-else>
		<div class="orders">
			<div class="orders-title">订单号:</div>
			<span class="orders-id">
				{{ latestOrderList[0].orderId }}
			</span>
			<Icon name="orders-o" size="20px" @click="copyOrderId(latestOrderList[0].orderId)" />
		</div>
		<ul class="order-wrapper">
			<li class="order-item" v-for="item in latestOrderList" :key="item.id">
				<Image width="1.2rem" height="1.2rem" fit="cover" :src="BASE_IMAGE_URL + item.image" />
		...
		<p class="description">
			共<span style="color: red">{{ allCount }}</span
			>件商品，实付<span style="color: red">{{ (price / 100).toFixed(2) }}</span
			>￥
		</p>
	</div>
</template>
...
const allCount = computed(() => {
	return props.latestOrderList.reduce((pre: number, item: any) => {
