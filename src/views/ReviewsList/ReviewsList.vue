<template>
	<div class="reviews-list">
		<NavBar title="商品评论" left-arrow @click-left="$router.back()" />
		<header>
		...
		/>
		<section class="reviews-wrapper" v-else>
			<List v-model:loading="loading" :finished="finished" :immediate-check="false" finished-text="没有更多了" @load="onLoad">
				<CellGroup inset class="reviews-content" v-for="item in reviewListInfo.reviewList" :key="item.id">
					<div class="user-content">
						<div class="user-avatar">
							<Image width="100%" height="100%" :src="BASE_URL + item.avatar" round fit="cover" @click="viewImage([BASE_URL + item.avatar])" />
						</div>
						<div class="user-info">
							<p class="user-name">
								{{ item.username }}
							</p>
							<Rate class="user-rete" v-model="item.rating" :size="18" readonly color="#ffd21e" void-icon="star" void-color="#eee" />
						</div>
					...
					<div class="store-review" v-if="item.storeReview !== null">
...
<script setup name="ReviewsList" lang="ts">
...
import { GetReviewCountType, GetReviewListType } from "@/api/interface/SendType";
import { reviewList, reviewType } from "@/views/types/interface";
const BASE_URL = import.meta.env.VITE_LOCAL_SERVE_IMGE_URL;
const loading = ref(false);
const finished = ref(false);
const reviewListInfo = reactive<{ total: number; reviewList: Array<reviewType>; currentPage: number }>({
	total: 0,
	reviewList: [],
	currentPage: 0
});
...
const getReviewList = async (reta: number, page = 0, pageSize = 10) => {
	const data: GetReviewListType = {
		reta,
		page,
		pageSize
	};
	if (route.query.type === "1") {
		data.dishId = route.query.id as string;
	} else {
		data.setmealId = route.query.id as string;
	}
	const res = await sendGetReviewList<null, reviewList | null>(data);
	loading.value = false;
...
const chooseReviews = (index: number, reta: number) => {
	active.value = index;
	reviewListInfo.reviewList = [];
	getReviewList(reta);
};
const getReviewsCount = async () => {
	const data: GetReviewCountType = {};
	if (route.query.type === "1") {
		data.dishId = route.query.id as string;
	} else {
		data.setmealId = route.query.id as string;
